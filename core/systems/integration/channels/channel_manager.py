...
class ChannelManager:
    ...
    def __init__(
        ...
        self._runtime = ChannelRuntime(channel_routes, workspace_dir=self.workspace_dir)
    ...
    def set_agent_callback(self, callback: Callable[[str, str], str]) -> None:
        ...
        # Propagate to wechat_claw if registered
        claw = self.channels.get("wechat_claw")
        if claw and hasattr(claw, "start_polling") and getattr(claw, "_running", False):
            claw._agent_callback = callback
    ...
    def set_route_callback(
        ...
        claw = self.channels.get("wechat_claw")
        if claw:
            setattr(claw, "_route_callback", callback)
