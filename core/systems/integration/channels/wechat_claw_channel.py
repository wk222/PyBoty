        ...
        while self._running and self._credentials:
            try:
                ...
                for raw_msg in data.get("msgs", []):
                    ...
                    parsed_dict = self.parse_incoming(raw_msg)
                    if parsed_dict and self._agent_callback:
                        try:
                            # Use rich route callback if available, otherwise fallback to basic agent callback
                            from .channel_runtime import ChannelMessage, ChannelRouteDecision
                            msg = ChannelMessage.from_payload(parsed_dict)
                            if msg is not None:
                                reply = ""
                                if hasattr(self, "_route_callback") and self._route_callback:
                                    ...
                                    reply = self._route_callback("wechat_claw", msg, decision)
                                else:
                                    reply = self._agent_callback(msg.thread_id, msg.message)
                                if reply:
                                    self.send_message(msg.user_id, reply)
                        except Exception as exc:
