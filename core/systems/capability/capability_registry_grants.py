    ...
    def issue_app_grant(
        ...
        *,
        ...
        grant = CapabilityGrant(
            ...
            metadata=dict(metadata or {}),
            # ``ttl_seconds <= 0`` means "issue an already-expired grant"
            # (handy for tests and for revocation-on-issue flows). Positive
            # TTLs are clamped to a 60s minimum to avoid races where a
            # caller would never be able to use the grant.
            expires_at=time.time() + (int(ttl_seconds) if int(ttl_seconds) <= 0 else max(60, int(ttl_seconds))),
        )
