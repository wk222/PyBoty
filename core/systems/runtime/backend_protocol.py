...
class ToolStorageProtocol(Protocol):
    """Protocol for managing persisted tool definitions."""
...
@runtime_checkable
class AgentStorageProtocol(Protocol):
    """Protocol for managing persisted subagent definitions."""

    def get_agent(self, name: str) -> Any | None: ...
    def list_agents(self) -> dict[str, str]: ...
    def add_agent(self, definition: Any) -> bool: ...
...
class AppManagerProtocol(Protocol):
    """Protocol for managing created web applications."""

    def get_app(self, name: str) -> Any | None: ...
    def list_apps(self) -> list[dict[str, Any]]: ...
    def create_app(self, name: str, **kwargs: Any) -> dict[str, Any]: ...
...
class SkillRegistryProtocol(Protocol):
    """Protocol for managing discovered skills and tools."""
...
class TaskSchedulerProtocol(Protocol):
    """Protocol for scheduling and managing workflow tasks."""
...
@runtime_checkable
class PyFlowEngineProtocol(Protocol):
    """Protocol for graph-based workflow execution."""

    def execute_graph(self, graph_id: str, inputs: dict[str, Any]) -> Any: ...


@runtime_checkable
class SubagentRegistryProtocol(Protocol):
    """Protocol for managing active subagent runs."""
...
@runtime_checkable
class SkillMarketplaceProtocol(Protocol):
    """Protocol for managing skill packages and distribution."""
    ...
    def install_from_github(self, repo: str, subpath: str = "") -> dict[str, Any]: ...
    def install_from_url(self, url: str) -> dict[str, Any]: ...


@runtime_checkable
class UnifiedAssetInventoryProtocol(Protocol):
    """Protocol for cross-domain asset indexing."""
