...
class ContextHygieneRuntime:
    ...
    def suggest_garden_updates(
        ...
        *,
        ...
        Disabled by default to avoid an extra synchronous LLM call per compaction.
        Enable by setting ``SummarizationConfig.enable_garden_suggestions = True``.
        """
        if summarize_fn is None or not summary.strip():
            return []
        if not getattr(self._config, "enable_garden_suggestions", False):
            return []
        ...
        try:
            ...
            try:
                ...
                logger.debug("Garden suggestion JSON parse failed, falling back to lines: %s", parse_exc)
                return [s.strip() for s in res.split("\n") if s.strip()]
        except Exception as exc:
            logger.debug("Garden suggestion LLM call failed: %s", exc)
            return []
