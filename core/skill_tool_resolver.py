...
from .skill_runtime import build_tool_from_definition, load_python_module_tools
...
def load_skill_tools(
    *,
    ...
    for tool in _iter_definition_tools(skill_name=skill_name, skill=skill):
        if tool.name in seen_names:
            continue
        seen_names.add(tool.name)
    ...
    if materialized_skill_dir is not None:
        for tool in load_python_module_tools(materialized_skill_dir, skill_name):
            if tool.name in seen_names:
                continue
            seen_names.add(tool.name)
            resolved_tools.append(tool)
...
def _iter_definition_tools(*, skill_name: str, skill: SkillDefinition):
    for tool_def in skill.tools:
        try:
            tool = build_tool_from_definition(tool_def, skill_name)
