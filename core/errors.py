...
Inspired by OpenClaw's tool error hierarchy and error utilities:
...
def extract_error_code(err: Any) -> str | None:
    ...
    Inspired by OpenClaw ``extractErrorCode``: handles
...
def redact_sensitive_text(text: str) -> str:
    ...
    Inspired by OpenClaw ``redactSensitiveText``.
    """
    result = text
...
def format_error(err: Any) -> str:
    ...
    Inspired by OpenClaw ``formatUncaughtError``.
    """
    if isinstance(err, ToolError):
