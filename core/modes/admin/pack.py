...
def _initialize_persistent_runtime_if_needed(host: Any) -> None:
    ...
    # Subscribe to telemetry reports — draft-only unless explicitly enabled
    def _on_telemetry_report(event: Event) -> None:
        if not host.admin:
            return
        if os.environ.get("PYBOT_ADMIN_TELEMETRY_AUTO_REPAIR", "").strip().lower() not in (
            "1",
            "true",
            "yes",
        ):
            logger.info(
                "Telemetry report received; auto-repair disabled "
                "(set PYBOT_ADMIN_TELEMETRY_AUTO_REPAIR=1 to enable)"
            )
            return
...
ADMIN_API_METHODS: dict[str, Callable[..., Any]] = {
    "start_admin_loop": _api_start_admin_loop,
    "stop_admin_loop": _api_stop_admin_loop,
    "submit_admin_goal": _api_submit_admin_goal,
    "plan_admin_goal": _api_plan_admin_goal,
