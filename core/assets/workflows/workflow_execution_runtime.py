...
class WorkflowExecutionRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        exec_node: Callable[[FlowNode, WorkflowDef, str], Any],
        ...
        self._error_callbacks: list[Callable[[WorkflowDef, FlowNode, Exception], None]] = []

    def add_error_callback(self, callback: Callable[[WorkflowDef, FlowNode, Exception], None]) -> None:
        """Register a callback invoked when ``notify_and_continue`` fires."""
        self._error_callbacks.append(callback)

    def _send_error_notification(self, workflow: WorkflowDef, node: FlowNode, error: Exception) -> None:
        for cb in self._error_callbacks:
            try:
                cb(workflow, node, error)
            except Exception:
                logger.warning(
                    "Error callback failed for node %s in workflow %s",
                    node.id, workflow.name, exc_info=True,
                )
        logger.warning(
            "[NOTIFY] Workflow '%s' node '%s' failed: %s — continuing execution",
            workflow.name, node.id, error,
        )
        ...
        for fp in sorted(self._history_dir.glob("run_*.json"), reverse=True)[: self._max_history]:
            try:
                ...
                rec = WorkflowRunRecord(
                    ...
                    root_mode=data.get("root_mode", "assistant"),
    ...
    @staticmethod
    def _workflow_session_context(workflow: WorkflowDef) -> tuple[str, str | None, str]:
        variables = workflow.variables if isinstance(workflow.variables, dict) else {}
        thread_id = str(variables.get("thread_id", "")).strip()
        session_key = str(variables.get("session_key", "")).strip() or None
        root_mode = str(variables.get("root_mode", variables.get("mode", "assistant"))).strip() or "assistant"
        return thread_id, session_key, root_mode
    ...
    def _record_workflow_run(
        ...
        *,
        ...
        if self._session_runtime is None:
            return
        thread_id, session_key, root_mode = self._workflow_session_context(workflow)
        if not thread_id:
            return
        self._session_runtime.record_workflow_run(
            thread_id=thread_id,
            ...
            status=status,
            source=run_record.source or "workflow.execution",
            preview=preview,
            root_mode=root_mode,
            metadata=metadata,
        )
    ...
    def run_workflow(self, workflow: WorkflowDef) -> dict[str, Any]:
        ...
        run_record = WorkflowRunRecord(
            ...
            root_mode=root_mode,
            source=str(workflow.variables.get("source", "workflow")).strip() or "workflow",
        ...
        self._record_workflow_run(
            ...
            status="running",
            preview=str(workflow_inputs)[:240],
        ...
        try:
            ...
            self._record_workflow_run(
                ...
                status="failed",
                preview=str(exc),
                metadata={"error": str(exc), "workflow_status": workflow.status.value},
            )
            self._run_history.append(run_record)
            self._persist_run(run_record)
            return {"status": "error", "error": str(exc), "run_id": run_record.run_id}
        ...
        try:
            ...
            self._record_workflow_run(
                ...
                run_record=run_record,
                status=run_record.status,
                preview=str(final.get("status", "")),
                metadata={"workflow_status": workflow.status.value},
            )
            self._run_history.append(run_record)
            self._persist_run(run_record)
            final["run_id"] = run_record.run_id
            return final
    ...
    def _execute_branch_node(
        ...
        *,
        ...
        try:
            run_id = self._current_run.run_id if self._current_run else "local"
            result = self._exec_node(node, workflow, run_id)
        ...
        except (WorkflowTimerPause, WorkflowSignalPause) as pause:
            node_rec.status = "waiting_timer" if isinstance(pause, WorkflowTimerPause) else "waiting_signal"
            self._record_node(node_rec, node)
            return self._timer_or_signal_response(pause)
        except Exception as exc:
    ...
    def _handle_node_failure(
        ...
        *,
        ...
        if strategy == OnErrorStrategy.NOTIFY_AND_CONTINUE:
            self._send_error_notification(workflow, node, error)
            node.status = NodeStatus.COMPLETED
            node.output = {"error": str(error), "_error_handled": True, "_notified": True}
            node.completed_at = time.time()
            workflow.variables[f"{node.id}.output"] = node.output
            workflow.variables[f"{node.id}.status"] = "completed"
            self._log_event(workflow, node.id, "error_notified", f"notify_and_continue: {error}")
            if self._process_node_success:
                self._process_node_success(workflow, node.id)
            return None
    ...
    def _timer_or_signal_response(pause: WorkflowTimerPause | WorkflowSignalPause) -> dict[str, Any]:
        if isinstance(pause, WorkflowTimerPause):
            return {
