...
from .skill_loading import render_skill_file_content
...
class SkillRegistry:
    ...
    def read_skill_file(self, name: str, relative_path: str) -> str | None:
        ...
        if skill is None:
            return None
        content = self.storage.read_file(skill, relative_path)
        return render_skill_file_content(skill, relative_path, content)

    async def aread_skill_file(self, name: str, relative_path: str) -> str | None:
        skill = self.skills.get(name)
        if skill is None:
            return None
        content = await self.storage.aread_file(skill, relative_path)
        return render_skill_file_content(skill, relative_path, content)
