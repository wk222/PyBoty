...
class SkillMarketplace:
    ...
    def install_from_url(self, url: str) -> dict[str, Any]:
        """Download and install a skill from a remote URL."""
        import tempfile
        try:
            ...
            result = self.install_skill(temp_path)
            os.unlink(temp_path)
            return result
        except Exception as e:
            return {"success": False, "error": f"Failed to download or install from URL: {e}"}

    def install_from_github(self, repo: str, branch: str = "main", subpath: str = "") -> dict[str, Any]:
        """Install a skill directly from a GitHub repository.
        Example: repo='user/repo', subpath='skills/my-skill'
        """
        import subprocess
        import tempfile
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                # Clone the specific branch (shallow clone for speed)
                clone_cmd = ["git", "clone", "--depth", "1", "-b", branch, f"https://github.com/{repo}.git", tmpdir]
                subprocess.run(clone_cmd, check=True, capture_output=True)
                
                source_dir = os.path.join(tmpdir, subpath) if subpath else tmpdir
                
                if not os.path.exists(source_dir):
                    return {"success": False, "error": f"Path '{subpath}' not found in repository"}
                
                if not os.path.exists(os.path.join(source_dir, "SKILL.md")):
                    return {"success": False, "error": "SKILL.md not found in the specified repository path"}
                
                # Package it into a .skill file
                package_path = self.package_skill(source_dir)
                if not package_path:
                    return {"success": False, "error": "Failed to package the cloned skill"}
                
                # Install it
                result = self.install_skill(package_path)
                return result
        except subprocess.CalledProcessError as e:
            return {"success": False, "error": f"Git clone failed: {e.stderr.decode('utf-8', errors='ignore')}"}
        except Exception as e:
            return {"success": False, "error": f"Failed to install from GitHub: {e}"}
...
class InstallSkillInput(BaseModel):
    package_path: str = Field(default="", description="技能包文件路径 (.skill)")
    url: str = Field(default="", description="远程 .skill 文件的 URL")
    github_repo: str = Field(default="", description="GitHub 仓库 (例如 'user/repo')")
    github_subpath: str = Field(default="", description="GitHub 仓库中技能所在的子目录")


class InstallSkillTool(BaseTool):
    name: str = "install_skill"
    description: str = """从本地 .skill 文件、远程 URL 或 GitHub 仓库安装技能。
必须且仅提供 package_path, url, github_repo 中的一个。"""
    args_schema: type[BaseModel] = InstallSkillInput
    marketplace: Any = Field(default=None, exclude=True)
    model_config = ConfigDict(arbitrary_types_allowed=True)

    def _run(self, package_path: str = "", url: str = "", github_repo: str = "", github_subpath: str = "") -> str:
        if github_repo:
            result = self.marketplace.install_from_github(repo=github_repo, subpath=github_subpath)
        elif url:
            result = self.marketplace.install_from_url(url=url)
        elif package_path:
            result = self.marketplace.install_skill(package_path)
        else:
            result = {"success": False, "error": "必须提供 package_path, url 或 github_repo"}
