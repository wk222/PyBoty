...
class SkillSource:
    ...
    def resolved(self) -> SkillSource:
        normalized_path = _resolve_skill_source_path(self.path, self.backend, self.flavor)
        return SkillSource(
            name=self.name,
            path=self.backend.normalize_root(normalized_path),
...
def _resolve_skill_source_path(path: str | Path, backend: SkillBackend, flavor: str) -> str | Path:
    """Resolve source roots for well-known external layouts."""
    if flavor != "openclaw" or not isinstance(backend, FilesystemSkillBackend):
        return path
    root = Path(path)
