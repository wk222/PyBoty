...
我们通过 1\,474 个自动化测试验证了系统的所有主要子系统，并报告治理中间件成功地在层级化多智能体集群中执行了单调能力约束。
\end{abstract}
...
\subsection{语义长期记忆}
\texttt{SemanticMemoryManager} 持久化智能体的观察、事实和决策。它采用复合评分机制（时效性、相关性、重要性），并包含去重（跳过近似相同的条目）和提示注入守卫，确保记忆存储随时间保持干净、安全且高度相关。

\subsection{MemoryDistill：记忆蒸馏流水线}
\label{sec:deep-digest}

Markdown 知识园林和语义记忆层提供了持久化存储，但它们没有解决持续将原始对话消息流转化为结构化、紧凑的长期知识的问题。
PyBot 引入了 \textbf{MemoryDistillManager}（\texttt{core/systems/memory/memory\_distill.py}，第 1 层）作为三阶段异步流水线，弥合原始会话历史和长期记忆之间的差距。

\begin{enumerate}[label=\textbf{阶段~\arabic*.}]
  \item \textbf{日记（Journal）。}在每个助手回合之后（由执行画布的蒸馏策略触发），最近对话消息的子集被传递给 LLM 驱动的摘要器，生成当天的"事件日记"条目（\texttt{memory/daily/YYYY-MM-DD.md}）。摘要器被指示将相关回合合并为事件，抑制琐碎交流，保留关键决策和制品路径。通过内容哈希集防止重复日记写入。日记条目使用 7 种分类标签（\texttt{[偏好]}、\texttt{[决策]}、\texttt{[事实]}、\texttt{[人物]}、\texttt{[工具]}、\texttt{[待办]}、\texttt{[其他]}），并显式提取工具调用结果。
  \item \textbf{蒸馏（Distill）。}周期性地（或当 \texttt{deep} 画布在每个回合触发时），流水线读取最近 $N$ 个每日日记文件和当前 \texttt{MEMORY.md}，调用第二次 LLM 传递来合并、去重和解决新观察与现有长期事实之间的冲突，并用结果规范事实集（上限 60 条）覆写 \texttt{MEMORY.md}。每条记忆条目被赋予重要性评分（\texttt{★★★}、\texttt{★★}、\texttt{★}），低重要性条目在空间紧张时优先删除。
  \item \textbf{归档（Archive）。}已处理的每日日记文件被移至 \texttt{memory/archive/}，防止在后续蒸馏运行中重新处理。
\end{enumerate}
...
流水线与现有的 \texttt{MemoryManager.get\_context\_prompt()} 路径集成：蒸馏后的 \texttt{MEMORY.md} 通过标准会话脊柱上下文装配注入系统提示，闭合临时对话与持久化记忆之间的循环。
...
PyBot 维护了一个通过 \texttt{pytest} 执行的\textbf{1\,474 个自动化测试}的综合测试套件。
测试覆盖所有主要子系统：
...
PyBot 完全开源（Apache~2.0），并通过覆盖所有主要子系统的 1\,474 个自动化测试进行了验证。
