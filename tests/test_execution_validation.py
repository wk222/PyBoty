...
def test_iterative_app_validator_reports_missing_app(temp_paths):
    validator = IterativeAppValidator(workspace_dir=str(temp_paths.workspace_dir))

    result = validator.validate(app_name="missing-app")

    assert result["success"] is False
    assert "不存在" in result["error"]


def test_iterative_app_validator_detects_html_and_api_failures(temp_paths):
    ...
    validator = IterativeAppValidator(workspace_dir=str(temp_paths.workspace_dir))
    result = validator.validate(app_name="demo")
